import type { Canvas } from './context'

export type { Canvas }

// A renderer-agnostic subset of ARIA roles for autocomplete, widened with
// `(string & {})` so any custom/other role string is still accepted. Replaces
// React's `AriaRole` so the DSL carries no React dependency.
type AriaRole =
	| 'alert'
	| 'alertdialog'
	| 'application'
...
function invokeRoleSingle(canvas: Canvas, config: LocatorConfig<'role'>, mode: QueryMode) {
	...
	if (mode === 'get') {
		return canvas.getByRole(config.arg, opts)
	}
	if (mode === 'find') {
		return canvas.findByRole(config.arg, opts)
	}
...
function invokeTextSingle(canvas: Canvas, config: LocatorConfig<'text'>, mode: QueryMode) {
	const opts = mergedOptions(config)
	if (mode === 'get') {
		return canvas.getByText(config.arg, opts)
	}
	if (mode === 'find') {
		return canvas.findByText(config.arg, opts)
	}
...
function invokeAll(canvas: Canvas, config: LocatorConfig, mode: 'get' | 'find') {
	if (config.type === 'role') {
		...
		if (mode === 'get') {
			return canvas.getAllByRole(config.arg, opts)
		}
	...
	const opts = config.options
		...
		: config.initialOptions
	if (mode === 'get') {
		return canvas.getAllByText(config.arg, opts)
	}
...
// `HTMLElement` may be absent outside a browser (e.g. building a locator in a
// Node context); guard so locator construction stays DOM-free and node-safe.
...
/** Query by ARIA role, optionally narrowed by accessible name. */
export const role = (roleName: AriaRole, name?: NameOption): FluentLocator<'role'> =>
	createLocator({
		type: 'role',
		arg: roleName,
...
/** Query by the `heading` role, optionally narrowed by accessible name. */
export const heading = (name?: NameOption): FluentLocator<'role'> => role('heading', name)
/** Query by the `button` role, optionally narrowed by accessible name. */
export const button = (name?: NameOption): FluentLocator<'role'> => role('button', name)
/** Query by the `link` role, optionally narrowed by accessible name. */
export const link = (name?: NameOption): FluentLocator<'role'> => role('link', name)
